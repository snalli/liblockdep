...
void _init() 
{
	...
	fprintf(fp," per task-struct memory footprint: %d bytes\n",
		sizeof(ldthread_t));
	fprintf(fp,"*************************************************************\n");	
} 
void _fini() 
...
{
	fprintf(fp,"\n=============================================\n");
	fprintf(fp,  "[ INFO: certain recursive locking detected ]\n");
	fprintf(fp,  "---------------------------------------------\n");
	fprintf(fp,"%s/%lu is trying to acquire lock:\n",
		curr->name,pthread_self());
	print_lock(next);
...
static int lock_release(ldthread_mutex_t *lock) { return 1; }
...
int ldthread_mutex_unlock(ldthread_mutex_t *mutex)
{
